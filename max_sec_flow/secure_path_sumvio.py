...
def secure_path_sumvio(nodes,arcs,capacity,security_labels):
    ...
    for h in commodities:
        for i, j in arcs:
            #m.addConstr(quicksum(flow[h, i, j] * abs(sec_classes[i] - sec_classes[j]) for i,j in arcs) <= gamma, 'secvio_%s_%s_%s' % (i, j, h))
            m.addConstr(flow[h, i, j] * abs(sec_classes[sourcenodes[h]] - sec_classes[j]) <= gamma,'secvio_%s_%s_%s' % (i, j, h))
